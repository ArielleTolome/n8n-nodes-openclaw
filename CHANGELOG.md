...
### Fixed
- SVG icon path: now correctly resolves to `dist/nodes/OpenClaw/openclaw.svg` alongside compiled JS
  (was incorrectly copying to `dist/OpenClaw/` — icon would not display in n8n)

### Added
- `npm audit --audit-level=high` step in GitHub Actions CI
- Updated README: 6 resources table, trigger security features, usableAsTool note
